...
function mergeObjects(objects) {
  return Object.fromEntries(
    objects.flatMap(Object.entries).reduce((acc, [key, value]) => {
...
function makeWord(lettersObject) {
  return Object.entries(lettersObject)
    .flatMap(([key, positionValues]) => positionValues.map((pos) => [pos, key]))
    .reduce((result, [pos, letter]) => {
      const lettersArray = [...result];
      lettersArray[pos] = letter;
      return lettersArray;
    }, [])
    .join('');
}
